import { ArrowLeft, Heart, Calendar, Package, ExternalLink, User, RefreshCw } from 'lucide-react';

interface ViewWishlistDetailsScreenProps {
  onNavigate: (screen: string) => void;
}

export function ViewWishlistDetailsScreen({ onNavigate }: ViewWishlistDetailsScreenProps) {
  const items = [
    {
      id: 1,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-green-50 p-4 py-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-gray-800">Detalhes da Lista</h1>
        </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-gray-600">Maria Santos</span>
                </div>
        ...
        <div className="bg-blue-100 border-2 border-blue-200 rounded-2xl p-4 mb-6">
          <div className="flex gap-3">
            ...
            <div>
              <div className="text-blue-800 mb-1">Suas Permissões</div>
              <p className="text-blue-700 text-sm">Você pode visualizar todos os itens e atualizar o status deles. Apenas o idoso pode adicionar, editar ou remover itens.</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {items.map((item) => (
            <div key={item.id} className="bg-white rounded-3xl shadow-lg p-6 hover:shadow-xl transition-all">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-sm border-2 ${getStatusColor(item.currentStatus)}`}>
                          {item.currentStatus}
                        </span>
                      </div>
              ...
              {/* Action Button - Only Status Update for Familiar */}
              <div className="flex gap-3 mt-4 pt-4 border-t border-gray-100">
                <button 
                  onClick={() => onNavigate('status-item')}
                  className="flex-1 bg-gradient-to-r from-purple-300 to-blue-300 text-gray-800 py-3 rounded-xl hover:from-purple-400 hover:to-blue-400 transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                >
